...
TOKEN = "4e9db6c0b454"  
MY_NUMBER = "917988236305"  # Replace with correct number
...
async def resume() -> str:
    resume_path = Path("/Users/aryanmathur/Downloads/Resume_latex_v2 (3).pdf")  # Change path if needed
    if not resume_path.exists():
        return "<error>Resume file not found.</error>"
    try:
        return convert_file(str(resume_path), 'md', extra_args=['--standalone'])
    except Exception as e:
        return f"<error>Failed to read or convert resume: {e}</error>"
...
async def fetch(
    ...
    start_index: Annotated[int, Field(default=0, description="On return output starting at this character index", ge=0)] = 0,
    raw: Annotated[bool, Field(default=False, description="Get raw HTML content")] = False,
) -> list[TextContent]:
